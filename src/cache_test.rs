...
async fn test_cache_expiration() {
    ...
    assert_eq!(cache.get().await, Some("expires soon".to_string()));
...
async fn test_cache_overwrite() {
    ...
    cache.set("first".to_string()).await;
    assert_eq!(cache.get().await, Some("first".to_string()));
...
async fn test_cache_empty_string() {
    ...
    assert_eq!(cache.get().await, Some(String::new()));
}
...
async fn test_cache_large_data() {
    ...
    assert_eq!(cache.get().await, Some(large_data));
}
...
async fn test_cache_concurrent_reads() {
    ...
    for handle in handles {
        ...
        assert_eq!(result, Some("shared data".to_string()));
    }
}
